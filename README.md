...
- 标题中禁止使用 emoji；如 `📖 Educations` 会生成 `-educations`，导致导航锚点不匹配
...
- 确认 `src/components/MarkdownPage.tsx` 中 `rehypePlugins` 包含 `rehypeRaw`
- 确认 `npm install` 已执行
